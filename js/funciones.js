...
function seleccionComboEmpresas(){
    ...
    eNuevoOriente.comboEmps.store.load();
    eNuevoOriente.comboEmps.focus();
    eNuevoOriente.comboEmps.expand();
}
...
function createLocalPrint(){ 
	$.getScript("localprint.js", function(){
		eno.localprint = new LocalPrint({ 
			id: 'customhandler', 
			version: '1.1.1',
			port: (8080*1),
			winPath: 'ImpresionLocal/LocalPrint.msi'//,
			//macPath: eno.HandlerMacPath,
			//debPath: eno.HandlerDebPath
		});
			
		eno.localprint.on('installed',function(){ 
			eno.localprint.getMac(); 
		}, this);
			
		eno.localprint.on('mac', function(response){
			Ext.apply(eno,{
...
function printImpresionTicket(scope, resp){
	 if(eno.localprint != undefined && Ext.isFunction(eno.localprint.directPrint)){			
        ...
        eno.localprint.directPrint(cfg);		
