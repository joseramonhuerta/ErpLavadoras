...
class Model {
    ...
    function jsDateTomysqli($jsDate){
        $date = "04/30/1973";
	...
	*/
    ...
    public function execute($query,$dbName=null){
        ...
        if (!$res) {
			if ($this->debug){	
				//throw new Exception('Debug: '. $this->name. "->".mysqli_error() ." dbName: ".$dbName." : ".$query);
				generaLog('query_'.$this->name,mysqli_error().":".$query);
				throw new Exception('Debug: '. $this->name. "->".mysqli_error()." $query");
			}else{
				generaLog('query_'.$this->name,mysqli_error().":".$query);				
	...
	*/
    public function insert($query,$dbName=null){
        ...
        if (!$res) { 
        	switch(mysqli_errno()){
        		case 1062:
					generaLog('insert_'.$this->name,mysqli_error().":".$query);
			...
			if ($this->debug){
				//throw new Exception('Debug: '. $this->name. "->".mysqli_error() );
				generaLog('insert_'.$this->name,mysqli_error().":".$query);
				throw new Exception('Debug: '. $this->name. "->".mysqli_error() . $query);
			}else{
				generaLog('insert_'.$this->name,mysqli_error().":".$query);
	...
	/*
	*	EJECUTA UN QUERY "UPDATE"	REGRESA TRUE AUNQUE DEBERIA REGRESAR  mysqli_affected_rows()); 
	*/
    public function update($query, $dbName=null) {
        ...
        if (!$res) {      
        	switch(mysqli_errno()){
        		case 1062:
        			generaLog('update_'.$this->name,mysqli_error().":".$query);
        			throw new Exception("El registro no puede duplicarse");
        		break;
        		default:        		
        	}  
	...
	function delete($Id){
		...
		if (!$result) {      
			if ($this->debug){
				generaLog('DELETE_'.$this->name,mysqli_error().":".$query);	
				throw new Exception('Debug: '. $this->name. "->".mysqli_error() . $query);
			}else{
				generaLog('DELETE_'.$this->name,mysqli_error().":".$query);
    ...
    /*
	*	EJECUTA UN QUERY "DELETE"	REGRESA TRUE AUNQUE DEBERIA REGRESAR  mysqli_affected_rows()); 
	*/
     public function queryDelete($query, $dbName=null) {
       // if ($this->singleton){
        ...
        if (!$res) {      
			if ($this->debug){
				throw new Exception('Debug: '. $this->name. "->".mysqli_error() . $query);
				generaLog('queryDelete_'.$this->name,mysqli_error().":".$query);
			//	throw new Exception('Debug: '. $this->name. "->".mysqli_error());
			}else{
				generaLog('queryDelete_'.$this->name,mysqli_error().":".$query);
				throw new Exception($this->name.": Error al intentar eliminar el registro, consulte con el administrador del sistema");
