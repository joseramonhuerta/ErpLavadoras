...
class ReporteInventariosPDF extends FPDF{
	var $cerrando=false;
	var $colorHeader;
	...
	function ReporteInventariosPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$Producto=$datos['filtros']['descripcion'];
		...
		$this->Cell(100,10,utf8_decode("Reporte de Inventario"),0,0,'L');	
		$this->SetX(108);
		...
		$this->Cell(20,$hCell,"Producto:",$border,0,'L');	#	Valor
		$this->SetFont($font,'',9);
		$this->Cell(20,$hCell,mb_strtoupper(UTF8_decode($Producto)),$border,0,'L');	#	Valor
	...
	function imprimeHeader(){
		...
		if ($this->imprimirHeader==true){
			...
			$this->Cell(40,5,utf8_decode("CODIGO"),$border,0,"C",$fill);		//------Header 
			$this->Cell(100,5,utf8_decode("DESCRIPCION"),$border,0,"C",$fill);		//------Header						
			$this->Cell(58,5,utf8_decode("ASIGNACION"),$border,0,"C",$fill);	
	...
	public function imprimeDetalles($datos){
		...
		foreach($datos['detalles'] as $dato){	
			...
			$codigo_barra = $dato['codigo_barra'];
			...
			$this->Cell(40,$heightCell,utf8_decode($codigo_barra),$border,$saltoDeLinea,'',$fill);
			$this->Cell(100,$heightCell,utf8_decode($descripcion),$border,$saltoDeLinea,'',$fill);
			$this->Cell(58,$heightCell,utf8_decode($asignacion),$border,$saltoDeLinea,'',$fill);	
