...
class ReporteGastosPDF extends FPDF{
	var $cerrando=false;
	var $colorHeader;
	...
	function ReporteGastosPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$this->Cell(100,10,utf8_decode("Reporte de Gastos"),0,0,'L');	
		$this->SetX(108);
	...
	function imprimeHeader(){
		...
		if ($this->imprimirHeader==true){
			...
			$this->Cell(20,5,utf8_decode("FOLIO"),$border,0,"C",$fill);		//------Header 
			$this->Cell(40,5,utf8_decode("FECHA"),$border,0,"C",$fill);	
			$this->Cell(110,5,utf8_decode("CONCEPTO"),$border,0,"C",$fill);		//------Header						
	...
	public function imprimeDetalles($datos){
		...
		foreach($datos['detalles'] as $dato){	
			...
			$this->Cell(110,$heightCell,utf8_decode($concepto),$border,$saltoDeLinea,'',$fill);
	...
	function imprimirTotales($ultimo=false){
		...
		$total = number_format ($datos['gastos']['total'] ,$decimales ,  '.' , ',' );
