...
class ReportePedidoTicket{
	...
	function crearReporte($datos,$formatos){
		...
		require "eko_framework/app/models/reporte_pedido_ticket_pdf.php";
		

		$pdf=new ReportePedidoTicketPDF('P','mm',array(250,70),$datos,$formatos);
		//--------------------------------------------------------------------------------------------------------
		$pdf->AddPage();
		// throw new Exception($formatos);
	...
	function obtenerDatos($params){
        ...
        $query = "SELECT p.id_pedido,DATE_FORMAT(p.fecha_entrega,'%d/%m/%Y') as fecha_entrega,DATE_FORMAT(p.fecha_ultimo_vencimiento,'%d/%m/%Y') as fecha_vencimiento,
		p.precio_renta,c.nombre as nombre_cliente, c.calle,c.colonia,t.nombre as nombre_trabajador, pp.descripcion, pp.codigo_barra
		FROM pedidos p 
		inner join cat_clientes c on c.id_cliente = p.id_cliente
		inner join cat_trabajadores t on t.id_trabajador = p.id_trabajador
		left join cat_productos pp on pp.id_producto = p.id_producto
