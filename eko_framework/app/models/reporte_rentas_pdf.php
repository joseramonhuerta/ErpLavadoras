...
class ReporteRentasPDF extends FPDF{
	var $cerrando=false;
	var $colorHeader;
	...
	function ReporteRentasPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$Cliente=$datos['filtros']['Cliente'];
		$Trabajador=$datos['filtros']['Trabajador'];
		...
		$this->Cell(100,10,utf8_decode("Reporte de Rentas"),0,0,'L');	
		$this->SetX(108);
		...
		$this->SetFont($font,'B',9);
		$this->Cell(12,$hCell,"Cliente:",$border,0,'L');	#	Valor
		$this->SetFont($font,'',9);
		$this->Cell(60,$hCell,mb_strtoupper(UTF8_decode($Cliente)),$border,0,'L');	#	Valor
		
		$this->SetFont($font,'B',9);
		$this->Cell(15,$hCell,"Cobrador:",$border,0,'L');	#	Valor
		$this->SetFont($font,'',9);
		$this->Cell(50,$hCell,mb_strtoupper(UTF8_decode($Trabajador)),$border,0,'L');	#	Valor
	...
	function imprimeHeader(){
		...
		if ($this->imprimirHeader==true){
			...
			$this->Cell(20,5,utf8_decode("RECIBO"),$border,0,"C",$fill);		//------Header 
			$this->Cell(40,5,utf8_decode("FECHA"),$border,0,"C",$fill);	
			$this->Cell(110,5,utf8_decode("CLIENTE"),$border,0,"C",$fill);		//------Header						
			$this->Cell(28,5,utf8_decode("TOTAL"),$border,0,"C",$fill);	
	...
	public function imprimeDetalles($datos){
		...
		foreach($datos['detalles'] as $dato){	
			...
			$this->Cell(20,$heightCell,utf8_decode($recibo),$border,$saltoDeLinea,'',$fill);
			$this->Cell(40,$heightCell,utf8_decode($fecha),$border,$saltoDeLinea,'',$fill);				
			$this->Cell(110,$heightCell,utf8_decode($nombre),$border,$saltoDeLinea,'',$fill);
			$this->Cell(28,$heightCell,utf8_decode($Total),$border,$saltoDeLinea,'R',$fill);	
	...
	function imprimirTotales($ultimo=false){
		...
		$total = number_format ($datos['rentas']['total'] ,$decimales ,  '.' , ',' );
