...
$json=array();
		...
		if($filtro > 0){
			$filtroString=" WHERE status_servicio={$filtro} AND (fecha >= ADDDATE(CURDATE(),-1) OR status_servicio NOT IN (6,7))";
			if($rol == 4)
				$filtroString .= " AND id_tecnico={$id_usuario}";
		}else{
			if($rol == 4)
				$filtroString .= " WHERE id_tecnico={$id_usuario} AND (fecha >= ADDDATE(CURDATE(),-1) OR status_servicio NOT IN (6,7))";
			else
				$filtroString .= " WHERE fecha >= ADDDATE(CURDATE(),-1) OR status_servicio NOT IN (6,7)";
		}	
