...
$json=array();
	if(isset($_GET["id_orden_servicio"])){
		...
		$consulta="SELECT o.id_orden_servicio, CAST(DATE_FORMAT(o.fecha,'%d/%m/%Y') as CHAR) as fecha, IFNULL(c.id_cliente, 0) as id_cliente, 
			...
			IFNULL(c2.nombre_cliente, '') as nombre_cliente_venta,
			o.status_servicio,CASE o.status_servicio WHEN 1 THEN 'RECIBIDO' WHEN 2 THEN 'EN REVISIÓN' WHEN 3 THEN 'COTIZADO' WHEN 4 THEN 'EN REPARACIÓN' WHEN 5 THEN 'REPARADO' WHEN 6 THEN 'ENTREGADO' WHEN 7 THEN 'DEVOLUCION' END AS status_servicio_descripcion,
			...
			o.descripcion_reparacion,o.importe_presupuesto,o.nombre_imagen,o.imagen,o.token, IFNULL(c.celular, o.celular) as celular,o.id_tecnico,o.nombre_imagen_back,o.imagen_back,o.nombre_imagen_tecnico,o.imagen_tecnico,CAST(DATE_FORMAT(o.fecha_entrega,'%d/%m/%Y') as CHAR) as fecha_entrega, o.tipo_servicio, IFNULL(o.id_cliente_venta, 0) as id_cliente_venta,IFNULL(o.id_puntodeventa,0) as id_puntodeventa,o.precio_venta		
